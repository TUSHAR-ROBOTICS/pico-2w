/*
  CubeSat Control Panel — Dual-Core IMU HTTP Server
  Raspberry Pi Pico 2W + ICM-20948

  Core 1 → reads ICM-20948 sensor continuously at ~100Hz
  Core 0 → runs WiFi + HTTP server, serves JSON and dashboard

  This separation means HTTP serving never blocks sensor reads
  and /data always returns fresh values instantly.

  Open browser at: http://<pico-ip>/
...
// Shared sensor state — written by Core1, read by Core0
// volatile ensures compiler never caches these in a register
volatile float g_roll    = 0;
volatile float g_pitch   = 0;
volatile int16_t g_ax    = 0;
volatile int16_t g_ay    = 0;
volatile int16_t g_az    = 0;
...
// ─── ICM register helpers (used by Core1) ────────────────────────────────────
...
// ════════════════════════════════════════════════════════════════════════════
// CORE 1 — Sensor loop (runs independently, never touches WiFi)
// ════════════════════════════════════════════════════════════════════════════
void setup1() {
  // Core1 uses same Wire bus — Wire.begin() already called in setup()
  // Just wait for Core0 to finish Wire init
  delay(500);
}
...
void loop1() {
  ...
  if (Wire.available() >= 6) {
    ...
    float roll  = atan2((float)ay, (float)az) * 57.2958f;
    float pitch = atan2(-(float)ax, sqrtf((float)ay*ay + (float)az*az)) * 57.2958f;

    // Low-pass filter
    g_roll  = g_roll  * 0.75f + roll  * 0.25f;
    g_pitch = g_pitch * 0.75f + pitch * 0.25f;
...
void loop() {
  ...
  // GET /data — return JSON with latest sensor values
  if (req.indexOf("GET /data") >= 0) {
    String json = "{";
    json += "\"roll\":"   + String((float)g_roll,  2) + ",";
    json += "\"pitch\":"  + String((float)g_pitch, 2) + ",";
    json += "\"ax\":"     + String((int)g_ax)          + ",";
    json += "\"ay\":"     + String((int)g_ay)          + ",";
    json += "\"az\":"     + String((int)g_az)          + ",";
